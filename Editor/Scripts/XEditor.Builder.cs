...
namespace EFramework.Unity.Editor
{
    public partial class XEditor
    {
        ...
        /// 更多信息请参考模块文档。
        /// </remarks>
        public partial class Builder : Tasks.Worker, Tasks.Panel.IOnGUI, Event.Internal.OnPreprocessBuild
        {
            /// <summary>
            ...
            /// Root 获取构建输出根目录路径。
            /// </summary>
            /// <returns>如果未通过 <see cref="RootAttribute"/> 自定义，则返回默认路径：项目目录/Builds/Installer</returns>
            public static string Root { get => Constants.GetCoustom<RootAttribute, string>(ref root, ref rootProp, XFile.PathJoin(XEnv.ProjectPath, "Builds", "Installer")); }
            ...
            void Event.Internal.OnPreprocessBuild.Process(params object[] args)
            {
                ...
                XLog.Debug("XEditor.Builder.OnPreprocessBuild: set define symbols: {0}.", string.Join(",", defines));
            }
            ...
            public override void Preprocess(Tasks.Report report)
            {
                ...
                if (!string.IsNullOrEmpty(ProfileFile))
                {
                    ...
                    {
                        XLog.Warn("XEditor.Builder.Preprocess: cannot found build profile file: {0}.", ProfileFile);
                    }
                    else
                    {
                        profile = AssetDatabase.LoadAssetAtPath<BuildProfile>(ProfileFile);
                        if (profile == null) throw new Exception($"XEditor.Builder.Preprocess: cannot load build profile file: {ProfileFile}");
                        profile = UnityEngine.Object.Instantiate(profile);
                        lastProfile = BuildProfile.GetActiveBuildProfile();
                        BuildProfile.SetActiveBuildProfile(profile);
                        XLog.Debug("XEditor.Builder.Preprocess: set build profile: {0}.", profile.name);
                    }
                }
#endif
            ...
            public override void Process(Tasks.Report report)
            {
                ...
                if (report.Current.Result == Tasks.Result.Succeeded) XLog.Debug("XEditor.Builder.Process: build <a href=\"file:///{0}\">{1}</a> succeeded.", Path.GetFullPath(File), Name);
                else XLog.Error("XEditor.Builder.Process: build <a href=\"file:///{0}\">{1}</a> failed.", Path.GetFullPath(File), Name);
                #endregion
                ...
                if (XFile.HasDirectory(brustSrc) || XFile.HasDirectory(symbolSrc))
                {
                    ...
                    XLog.Debug("XEditor.Builder.Process: backup symbols of <a href=\"file:///{0}\">{1}</a> succeeded.", Path.GetFullPath(symbolZip), Name);
                }
                #endregion
            }
            ...
            public virtual bool Run(string path = "", string name = "")
            {
                    ...
                    XEnv.Platform == XEnv.PlatformType.macOS)
                {
                    ...
                    {
                        XLog.Error("XEditor.Builder.Run: cannot found executable file: {0}.", path);
                        return false;
                    }
                    else
                    {
                        ...
                        XLog.Debug("XEditor.Builder.Run: running <a href=\"file:///{0}\">{1}</a>.", Path.GetFullPath(bin), name);
                        return true;
                    }
                }
                else if (XEnv.Platform == XEnv.PlatformType.Android)
                {
                    if (installing)
                    {
                        XLog.Error("XEditor.Builder.Run: previous is still installing.");
                        return false;
                    }
                    ...
                    installing = true;
                    XLog.Debug("XEditor.Builder.Run: installing <a href=\"file:///{0}\">{1}</a>.", Path.GetFullPath(path), name);

                    XLoom.RunAsync(() =>
                    {
                        var task = Command.Run(bin: Command.Find("adb", sdkRoot), args: new string[] { "install -r", path });
                        task.Wait();
                        installing = false;
                        if (task.Result.Data.Contains("Success"))
                        {
                            ...
                            task = Command.Run(bin: Command.Find("adb", sdkRoot), args: new string[] { "-d shell", "am start -a android.intent.action.MAIN -c android.intent.category.LAUNCHER -S -f 0x10200000 -n", activity });
                            task.Wait();
                        }
                    });
                    return true;
                ...
                {
                    XLoom.RunAsync(() => Command.Run(bin: "open", args: new string[] { XFile.PathJoin(path, "Unity-iPhone.xcodeproj") }).Wait());
                    return true;
                }
                else
                {
                    XLog.Warn("XEditor.Builder.Run: non supported platform: {0}.", XEnv.Platform);
                    return false;
                }
            }
