...
/// <summary>
/// TestXEditorBuilder 是 XEditor.Builder 的单元测试。
/// </summary>
public class TestXEditorBuilder
{
    ...
    {
        public const string Scene = "Assets/Temp/TestXEditorBuilder/Test.unity";
    ...
    [TestCase(typeof(XEditor.Builder), Description = "验证默认构建类的参数生成")]
    [TestCase(typeof(MyBuilder), Description = "验证自定义构建类的参数覆盖")]
    public void Prepare(Type type)
    {
        XLog.Debug("XEditor.Builder.Test.Prepare: testing build handler type: {0}.", type.Name);
        var handler = (XEditor.Builder)Activator.CreateInstance(type);
        try
        {
            handler.Preprocess(new XEditor.Tasks.Report());
            if (type == typeof(XEditor.Builder))
            {
                // 验证默认参数
                ...
                var expectedOutput = XFile.PathJoin(XEditor.Builder.Root, XEnv.Channel, XEnv.Platform.ToString());
            ...
            {
                // 验证自定义参数
                Assert.That(handler.Output, Is.EqualTo(XFile.PathJoin(XEditor.Builder.Root, "CustomOutput")), "应使用自定义的输出路径");
        ...
        finally
        {
            ...
            {
                XLog.Debug("XEditor.Builder.Test.Prepare: cleaning up custom output directory.");
    ...
    public void Execute()
    {
        var testDir = XFile.PathJoin(XEnv.ProjectPath, "Temp", "TestXEditorBuilder");
        ...
        try
        {
            ...
            // 构建阶段
            var worker = XEditor.Tasks.Workers["Builder Test/MyBuilder2"] as MyBuilder2;
            ...
            // var symbolZip = XFile.PathJoin(XEditor.Builder.Root, "Symbol", XEnv.Channel, XEnv.Platform.ToString(), handler.Name + ".zip");
