...
namespace EFramework.Unity.Editor
{
    public partial class XEditor
    {
        ...
        public class Command
        {
            ...
            /// <summary>
            /// batchMode 表示是否处于批处理模式。
            /// </summary>
            internal static bool batchMode;

            /// <summary>
            /// OnInitialize 初始化命令执行环境。
            /// </summary>
            [InitializeOnLoadMethod]
            internal static void OnInitialize() { batchMode = Application.isBatchMode; }

            /// <summary>
            /// Find 查找命令的完整路径。支持在系统 PATH、环境变量和自定义路径中查找，自动适配不同操作系统的命令后缀。
            /// </summary>
            /// <param name="bin">要查找的命令名称，如 git、python 等。</param>
            ...
            /// </remarks>
            public static string Find(string bin, params string[] extras)
            {
                if (string.IsNullOrEmpty(bin)) return string.Empty;

                var names = new string[] { bin };
                if (!bin.Contains("."))
                {
                    if (Application.platform == RuntimePlatform.WindowsEditor)
                    {
                        names = new string[] { bin + ".cmd", bin + ".exe" };
                    }
                    else if (Application.platform == RuntimePlatform.OSXEditor)
                    {
                        names = new string[] { bin, bin + ".app" };
                    }
                }
            ...
            public static Task<Result> Run(string bin, string cwd, bool print, bool progress, Dictionary<string, string> envs, params string[] args)
            {
                ...
                return Task.Run(() =>
                {
                    ...
                    if (print)
                    {
                        ...
                        if (proc.ExitCode != 0) XLog.Error("XEditor.Command.Run: finish {0} with code: {1}", name, proc.ExitCode);
                        else XLog.Debug("XEditor.Command.Run: finish {0} with code: {1}", name, proc.ExitCode);
                    }
                    else
                    {
