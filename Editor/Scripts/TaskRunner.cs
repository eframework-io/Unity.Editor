...
namespace EFramework.Unity.Editor
{
    ...
    internal class TaskRunner : EditorWindow
    {
        ...
        internal void OnDisable()
        {
            ...
            // 若当前项目的首选项发生变更，则主动保存
            if (XPrefs.Asset.Dirty && XFile.HasFile(XPrefs.IAsset.Uri)) XPrefs.Asset.Save();
        }
        ...
        internal void OnGUI()
        {
            ...
            var preferencesContent = $"[Preferences: {preferencesName}/{XEnv.Channel}/{XEnv.Version}/{XEnv.Mode}/{XLog.Level()}]";
            var preferencesDirty = string.IsNullOrEmpty(XPrefs.IAsset.Uri) || !XFile.HasFile(XPrefs.IAsset.Uri);
            var ocolor = GUI.color;
            if (preferencesDirty) GUI.color = Color.gray;
            if (EditorGUILayout.LinkButton(new GUIContent(preferencesContent))) XEditor.Preferences.Open();
            GUI.color = ocolor;
                                ...
                                if (taskFoldout.Status)
                                {
                                    ...
                                    if (taskMeta.Params != null && taskMeta.Params.Count > 0)
                                    {
                                        ...
                                        foreach (var param in taskMeta.Params)
                                        {
                                            ...
                                            var opvalue = marguments.ContainsKey(param) ? marguments[param] : param.Persist ? XPrefs.GetString(param.ID, param.Default) : param.Default;
