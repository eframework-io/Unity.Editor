...
/// <summary>
/// TestXEditorTasksRun 是 XEditor.Tasks.Run 的单元测试。
/// </summary>
        ...
        try
        {
            switch (testCase)
            {
                ...
                case "Params":
                    {
                        // 设置 XPrefs 中的参数值
                        XPrefs.Asset.Set(task1Meta.Params[0].ID, "value1");
                        XPrefs.Asset.Set(task2Meta.Params[0].ID, "value2");

                        // 测试场景1：完全使用 EditorPrefs 值
                        XEnv.ParseArgs(reset: true, new[] {
                            "-runTasks",
                            "-taskResults", batchReport,
                            "-taskID", "Test/Test Task1",
                        ...
                        Assert.That(task1.testParam, Is.EqualTo("value1"), "Should use value from XPrefs.Asset");
                        Assert.That(task2.testParam, Is.EqualTo("value2"), "Should use value from XPrefs.Asset");

                        // 测试场景2：命令行参数覆盖 EditorPrefs 值
                        XEnv.ParseArgs(reset: true, new[] {
                            "-runTasks",
                            "-taskResults", batchReport,
                            "-taskID", "Test/Test Task1",
                        ...
                        Assert.That(task1.testParam, Is.EqualTo("cmd1"), "Command line should override EditorPrefs");
                        Assert.That(task2.testParam, Is.EqualTo("cmd2"), "Command line should override EditorPrefs");

                        // 测试场景3：混合使用（一个用命令行，一个用 EditorPrefs）
                        XEnv.ParseArgs(reset: true, new[] {
                            "-runTasks",
                            "-taskResults", batchReport,
                            "-taskID", "Test/Test Task1",
                        ...
                        Assert.That(task2.testParam, Is.EqualTo("value2"), "Should fallback to EditorPrefs value");
