...
namespace EFramework.Unity.Editor
{
    public partial class XEditor
    {
        ...
        public partial class Event
        {
            ...
            internal static void OnInitialize()
            {
                ...
                foreach (var type in evts)
                {
                    ...
                    if (target == null || !target.Singleton)
                    {
                        ...
                        {
                            XLog.Error("XEditor.Event.OnInitialize: create instance of {0} failed: {1}", type.FullName, e.Message);
                        }
                    }
                    ...
                    if (target != null)
                    {
                        ...
                        var ids = type.GetInterfaces();
                        foreach (var id in ids)
                        {
                            if (ctype != id && ctype.IsAssignableFrom(id))
                            {
                                if (Callbacks.TryGetValue(id, out List<Type> callbacks) == false)
                                {
                ...
                XLog.Debug("XEditor.Event.OnInitialize: parsed {0} event(s) with {1} singleton(s).", Callbacks.Count, Singletons.Count);
            ...
            /// 通知指定类型的事件，触发对应的回调处理。
            /// </summary>
            /// <param name="id">事件标识类型。</param>
            /// <param name="args">传递给回调的参数列表。</param>
            /// <exception cref="ArgumentNullException">当事件标识为空时抛出。</exception>
            public static void Notify(Type id, params object[] args)
            {
                if (id == null) throw new ArgumentNullException("id");

                var ename = id.FullName.Replace("+", ".");
                if (Callbacks.TryGetValue(id, out var callbacks))
                {
                    if (callbacks != null && callbacks.Count > 0)
                    {
                        ...
                        for (int i = 0; i < Batches.Count; i++)
                        {
                            ...
                            //catch (Exception e)
                            //{
                            //    var error = XString.Format("XEditor.Event.Notify: invoke id={0} of callback={1} error: {2}", id, callback != null ? callback.GetType().FullName : "Null", e.Message);
                            //    XLog.Panic(e, error);
                            //    XLog.Error(error);
                            //}
