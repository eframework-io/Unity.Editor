...
namespace EFramework.Unity.Editor
{
    public partial class XEditor
    {
        ...
        public class Icons : EditorWindow
        {
            ...
            /// 创建并打开编辑器图标窗口
            /// </summary>
            [MenuItem("Assets/Editor Icons")]
            public static void Open()
            {
#if UNITY_2018
                var w = GetWindow<EditorIcons>("Editor Icons");
#else
                var w = CreateWindow<Icons>("Editor Icons");
#endif
                w.ShowUtility();
            ...
            private void OnEnable()
            {
                ...
                var all_icons = List.Where(x => GetIcon(x) != null);
                ...
                foreach (Texture2D x in Resources.FindObjectsOfTypeAll<Texture2D>())
                {
                    ...
                    if (icoContent == null) continue; // 跳过无效图标
                                                      //{ 
                                                      //    skipped_nulls++; 
                                                      //    continue; 
                                                      //}
                ...
                // 将找到的独特图标添加到图标列表
                List = List.ToList().Concat(unique).ToArray();
            ...
            /// <summary>
            /// 设置GUIStyle的所有背景为指定纹理
            /// </summary>
            /// <param name="s">要设置的GUIStyle</param>
            ...
            void InitIcons()
            {
                ...
                {
                    GUIContent ico = GetIcon(List[i]);

                    if (ico == null)
                    {
                        iconMissingNames.Add(List[i]);
                        continue;
                    }
